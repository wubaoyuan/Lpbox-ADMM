...
function [x_sol,best_sol,obj_list,constraint_violation,y1,y2,y3,time_elapsed] = ADMM_bqp_linear_eq_ineq(A,b,C,d,E,f,all_params)
%% default params
...
while constraint_stopping && iter<=max_iters && std_obj>=std_threshold
    ...
    tic; 
    [x_sol,cg_flag] = pcg(2*A+rho4*Esq+rho3*Csq+sparse(1:n,1:n,rho1+rho2,n,n),...
        -(b+z1+z2+E'*z4+C'*z3)+rho1*y1+rho2*y2+rho3*C'*d+rho4*E'*(f-y3),...
