%% min_x  x'*A*x+b'*x+gamma*x'*(1-x)-mu*sum_i (ln(x_i)+ln(1-x_i))  s.t. 0<=x<=1 and Cx=d 
%
...
function [x_sol,fval] = GSA_BQP_linear_eq(A,b,C,d,gamma,lg,mu,lm,x0)
n = numel(b);
obj_list = [];rel_list = []; do_stop = false;
options = optimoptions('fmincon','GradObj','on','Display','final','Hessian','lbfgs',...
    'TolFun',1e-4,'TolX',1e-4);

while ~do_stop
    [x_sol,fval] = fmincon(@(x)cost(x,A,b,gamma,mu),x0,[],[],C,d,zeros(n,1),ones(n,1),[],options);
