function [X, obj] = laicLPClus(Hs, wClus, T, C, bas, lam, weis)
...
for c = 1 : m
    ...
    IdxNs{c} = IdxN;
    for i = 1 : k
        f(IdxP(i, :)) = f(IdxP(i, :)) * weis(i) * wClus(c);
        f(IdxN(i, :)) = f(IdxN(i, :)) * weis(i) * wClus(c);
    end
end
...
%         continue;
%     end    
%     obj2 = obj2 + sum(sum(XPs{i} + XNs{i}, 2) .* weis(:)) * wClus(i);
% end
% equal('obj', obj, obj2);